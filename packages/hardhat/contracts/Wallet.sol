...
contract Wallet {
    ...
    function init(address _signer) external payable {
        ...
        assembly {
            initSigner := sload(signerSlot)
        }
        require(initSigner == address(0), "Signer already defined");
        assembly {
            sstore(signerSlot, _signer)
        }
    }
    ...
    function relay(
        ...
        bytes32 id = keccak256(abi.encodePacked(address(this), _implementation, keccak256(_data)));
    ...
    function _decodeReceipt(bytes32 _receipt)
        internal
        pure
        returns (
            bool _canceled,
