...
function encodeImpData(dependencies, to, value, data, minGasLimit, maxGasPrice, salt, expiration) {
...
function calcId(wallet, implementation, data) {
  return web3.utils.soliditySha3(
    ...
    { t: "bytes32", v: web3.utils.soliditySha3({ t: "bytes", v: data }) },
  );
}
...
contract("Marmo wallets", function (accounts) {
  ...
  describe("Relay intents", function () {
    it("Should relay signed tx, send ETH", async function () {
      ...
      const callData = encodeImpData(dependencies, to, value, data, minGasLimit, maxGasPrice, salt, expiration);

      const id = calcId(wallet.address, marmoImp.address, callData);
      ...
      await wallet.relay(marmoImp.address, callData, signature);

      bn(await web3.eth.getBalance(wallet.address)).should.be.a.bignumber.that.equals(bn(0));
      bn(await web3.eth.getBalance(accounts[9]))
        .sub(prevBalanceReceiver)
    ...
    it("Should fail to relay if transaction is wronly signed", async function () {
      ...
      await Helper.tryCatchRevert(wallet.relay(marmoImp.address, callData, signature), "Invalid signature");

      await Helper.tryCatchRevert(wallet.relay(marmoImp.address, callData, "0x"), "Invalid signature");
    ...
    it("Should relay is dependencies are filled", async function () {
      ...
      const did = calcId(wallet.address, marmoImp.address, dcallData);

      await web3.eth.sendTransaction({ from: accounts[0], to: wallet.address, value: 1 });

      const dsignature = signHash(did, privs[1]);
      await wallet.relay(marmoImp.address, dcallData, dsignature);
      ...
      bn(await web3.eth.getBalance(wallet.address)).should.be.a.bignumber.that.equals(bn(0));
      bn(await web3.eth.getBalance(accounts[8]))
        .sub(prevBalanceReceiver)
    ...
    it("Should fail to relay if dependencies are not filled", async function () {
      ...
      const signature = signHash(id, privs[1]);
      await Helper.tryCatchRevert(wallet.relay(marmoImp.address, calldata, signature), "Dependency is not satisfied");
    });
    it("Should fail to relay is intent is already relayed", async function () {
      ...
      await Helper.tryCatchRevert(wallet.relay(marmoImp.address, calldata, signature), "Intent already relayed");
    });
    it("Should relay sending intent from signer (without signature)", async function () {
      ...
      await wallet.relay(marmoImp.address, calldata, "0x", {
        from: accounts[1],
      });
    ...
    it("Should fail to relay with low gas limit", async function () {
      ...
      await Helper.tryCatchRevert(
        wallet.relay(marmoImp.address, calldata, signature, {
          gas: 100000,
        }),
        "",
      );
    ...
    it("Should fail to relay with high gas price", async function () {
      ...
      const signature = signHash(id, privs[1]);
      await Helper.tryCatchRevert(wallet.relay(marmoImp.address, calldata, signature), "Gas price too high");
    ...
    it("Should fail to relay if expired", async function () {
      ...
      const signature = signHash(id, privs[1]);
      await Helper.tryCatchRevert(wallet.relay(marmoImp.address, calldata, signature), "Intent is expired");
    ...
    it("Should relay with multiple dependencies", async function () {
      ...
      const id1Dependency = calcId(wallet.address, marmoImp.address, calldata1Dependency);
      ...
      const id2Dependency = calcId(wallet2.address, marmoImp.address, calldata1Dependency);

      const d1signature = signHash(id1Dependency, privs[1]);
      const d2signature = signHash(id2Dependency, privs[2]);

      await wallet.relay(marmoImp.address, calldata1Dependency, d1signature);

      await wallet2.relay(marmoImp.address, calldata2Dependency, d2signature);
    ...
    it("Should fail relay with multiple dependencies, if one is not filled", async function () {
      ...
      const id2Dependency = calcId(wallet2.address, marmoImp.address, calldata2Dependency);
  ...
  describe("Cancel intents", function () {
    it("Should cancel intent and fail to relay", async function () {
      ...
      const cid = calcId(wallet.address, marmoImp.address, ccalldata);

      const csignature = signHash(cid, privs[1]);

      await wallet.relay(marmoImp.address, ccalldata, csignature);

      (await wallet.isCanceled(id)).should.be.equals(true);

      // Try to relay transfer
      await Helper.tryCatchRevert(wallet.relay(marmoImp.address, calldata, signature), "Intent was canceled");
    ...
    it("Should fail to cancel intent if already relayed", async function () {
      ...
      const cancelReceipt = await wallet.relay(marmoImp.address, ccalldata, csignature);
    ...
    it("Should fail to cancel intent if already canceled", async function () {
      ...
      const c2id = calcId(wallet.address, marmoImp.address, c2calldata);

      const csignature = signHash(cid, privs[1]);
      const c2signature = signHash(c2id, privs[1]);

      const cancelReceipt1 = await wallet.relay(marmoImp.address, ccalldata, csignature);

      const cancelReceipt2 = await wallet.relay(marmoImp.address, c2calldata, c2signature);
  ...
  describe("Destroy", function () {
    it("Wallet is destroyable", async function () {
      ...
      let callData = encodeImpData(dependencies, to, value, data, minGasLimit, maxGasPrice, salt, expiration);

      let id = calcId(wallet.address, marmoImp.address, callData);
      ...
      // Destroy wallet
      id = calcId(wallet.address, destructImp.address, "0x00");

      await wallet.relay(destructImp.address, "0x00", signHash(id, privs[6]));

      // Wallet should be destroyed
      // should fail to send tokens
      salt = "0x01";
      callData = encodeImpData(dependencies, to, value, data, minGasLimit, maxGasPrice, salt, expiration);

      id = calcId(wallet.address, marmoImp.address, callData);
      ...
      await wallet.relay(marmoImp.address, ogcalldata, ogsignature);
